...
#define GYM_BACKGROUND \
    CLITERAL(Color) { 0x18, 0x18, 0x18, 0xFF }
...
struct Gym_Layout_Stack {
    Gym_Layout* items;
    size_t count;
    size_t capacity;
};

inline void gym_layout_stack_push(Gym_Layout_Stack* ls, Gym_Layout_Orient orient, Gym_Rect rect, size_t count, float gap);
...
inline void gym_render_nn(NN nn, Gym_Rect r) {
    Color low_color = RED;
        ...
        for(size_t i = 0; i < nn.as[l].cols; ++i) {
            ...
            if(l + 1 < nn.arch_count) {
                ...
                for(size_t j = 0; j < nn.as[l + 1].cols; ++j) {
                    ...
                    float value = sigmoidf(nn.ws[l][i][j]);
            ...
            if(l > 0) {
                high_color.a = floorf(255.f * sigmoidf(nn.bs[l - 1][i]));
...
inline void gym_render_mat_as_heatmap(Mat m, Gym_Rect r, size_t max_width) {
    Color low_color = RED;
    Color high_color = DARKBLUE;
    ...
    for(size_t y = 0; y < m.rows; ++y) {
        for(size_t x = 0; x < m.cols; ++x) {
            high_color.a = floorf(255.f * sigmoidf(m[y][x]));
...
inline void gym_render_nn_weights_heatmap(NN nn, Gym_Rect r) {
    size_t max_width = 0;
...
inline void gym_render_nn_activations_heatmap(NN nn, Gym_Rect r) {
    size_t max_width = 0;
    ...
    for(size_t i = 0; i < nn.arch_count; ++i)
        gym_render_mat_as_heatmap(nn.as[i].as_mat(), gym_layout_slot(), max_width);
    gym_layout_end();
}

inline void gym_plot(Font font, Gym_Plot plot, Gym_Rect r, Color c) {
    ...
    DrawTextEx(font, "0", {r.x + 0, y0 - r.h * 0.04f}, r.h * 0.04, 0, WHITE);

    if(plot.count > 0) {
        char buffer[64];
        snprintf(buffer, sizeof(buffer), "%f", plot.items[plot.count - 1]);
        DrawTextEx(font, buffer, {r.x, r.y}, r.h * 0.08, 0, WHITE);
    }
}

inline void gym_slider(float* value, bool* dragging, float rx, float ry, float rw, float rh) {
    float knob_radius = rh;
    Vector2 bar_size = {
        .x = rw - 2 * knob_radius,
        .y = static_cast<float>(rh * 0.25),
    };
...
inline void gym_nn_image_grayscale(NN nn, void* pixels, size_t width, size_t height, size_t stride, float low, float high) {
    GYM_ASSERT(nn.input().cols >= 2);
    GYM_ASSERT(nn.output().cols >= 1);
    uint32_t* pixels_u32 = (uint32_t*)pixels;
    for(size_t y = 0; y < height; ++y) {
        for(size_t x = 0; x < width; ++x) {
            nn.input()[0] = (float)x / (float)(width - 1);
            nn.input()[1] = (float)y / (float)(height - 1);
            nn.forward();
            float a = nn.output()[0];
            if(a < low) a = low;
            if(a > high) a = high;
